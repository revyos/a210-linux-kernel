...
static int th1520_i2s_set_fmt_dai(struct snd_soc_dai *cpu_dai, unsigned int fmt)
{
	...
	case SND_SOC_DAIFMT_RIGHT_J:
		...
		cnfin |= CNFIN_RSAFS_RIGHT_JUSTIFIED;
		break;
	case SND_SOC_DAIFMT_LEFT_J:
		cnfout |= IISCNFOUT_TSAFS_LEFT_JUSTIFIED;
		cnfin |= CNFIN_RSAFS_LEFT_JUSTIFIED;
		break;
	case SND_SOC_DAIFMT_DSP_B:
		cnfout |= IISCNFOUT_TSAFS_PCM;
	...
	if (i2s_private->alolrc_high) {
		cnfout |= IISCNFOUT_TALOLRC_HIGHFORLEFT;
