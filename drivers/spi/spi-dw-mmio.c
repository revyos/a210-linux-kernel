...
static int dw_spi_mmio_probe(struct platform_device *pdev)
{
	...
	/* Get DR (Data Register) address map size */
	if (device_property_read_u32(&pdev->dev, "reg-map-size",
				     &dws->reg_map_size))
		dws->reg_map_size = DW_SPI_DR_MAP_SIZE;

	/* Get dma tx/rx address Increment */
	dws->txdst_incr = of_property_read_bool(pdev->dev.of_node, "dma-tx-addr-incr");
	dws->rxsrc_incr = of_property_read_bool(pdev->dev.of_node, "dma-rx-addr-incr");
