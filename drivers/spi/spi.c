	...
	list_for_each_entry(xfer, &msg->transfers, transfer_list) {
		...
		if (xfer->tx_buf != NULL) {
			ret = spi_map_buf_attrs(ctlr, tx_dev, &xfer->tx_sg,
						(void *)xfer->tx_buf,
						xfer->len_aligned ? xfer->len_aligned : xfer->len, DMA_TO_DEVICE,
						attrs);
			if (ret != 0)
				return ret;
		}
		...
		if (xfer->rx_buf != NULL) {
			ret = spi_map_buf_attrs(ctlr, rx_dev, &xfer->rx_sg,
						xfer->rx_buf, xfer->len_aligned ? xfer->len_aligned : xfer->len,
		...
		list_for_each_entry(xfer, &msg->transfers, transfer_list) {
			if ((ctlr->flags & SPI_CONTROLLER_MUST_TX) &&
			    !xfer->tx_buf) {
					...
					if (ctlr->can_dma && ctlr->can_dma(ctlr, msg->spi, xfer) && ctlr->dma_alignment > 0) {
						max_tx = ALIGN(max_tx, ctlr->dma_alignment);
						if(!IS_ALIGNED(xfer->len, ctlr->dma_alignment)) {
							xfer->len_aligned = ALIGN(xfer->len, ctlr->dma_alignment);
						}
					}
				}
			if ((ctlr->flags & SPI_CONTROLLER_MUST_RX) &&
			    !xfer->rx_buf) {
					...
					if (ctlr->can_dma && ctlr->can_dma(ctlr, msg->spi, xfer) && ctlr->dma_alignment > 0) {
						max_rx = ALIGN(max_rx, ctlr->dma_alignment);
						if(!IS_ALIGNED(xfer->len, ctlr->dma_alignment)) {
							xfer->len_aligned = ALIGN(xfer->len, ctlr->dma_alignment);
						}
					}
