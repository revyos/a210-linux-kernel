...
static int dw_spi_dma_config_tx(struct dw_spi *dws)
{
	...
	struct axi_dma_peripheral_config tx_pconfig = {0};
	...
	if (dws->txdst_incr) {
		tx_pconfig.dst_addr_incr = true;
		txconf.peripheral_config = &tx_pconfig;
		txconf.peripheral_size = sizeof(tx_pconfig);
	}
...
static int dw_spi_dma_config_rx(struct dw_spi *dws)
{
	...
	struct axi_dma_peripheral_config rx_pconfig = {};
	...
	if (dws->rxsrc_incr) {
		rx_pconfig.src_addr_incr = true;
		rxconf.peripheral_config = &rx_pconfig;
		rxconf.peripheral_size = sizeof(rx_pconfig);
	}
