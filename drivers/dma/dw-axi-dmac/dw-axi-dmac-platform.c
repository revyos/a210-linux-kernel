 ...
 *
 * Return:
 *   - Corresponding DWAXIDMAC_BURST_TRANS_LEN_* enum value for valid inputs
 *   - DWAXIDMAC_BURST_TRANS_LEN_4 (with warning) for invalid inputs
 *
 * Notes:
 *   - Emits a once-only warning for unsupported burst length values
 *   - Default return value (DWAXIDMAC_BURST_TRANS_LEN_4) is chosen as a safe
 *     fallback for most DMA operations
 *   - Callers should validate burst length before use if possible
 */
static inline unsigned int dma_burst_len_to_enum(unsigned int burst_len)
{
    switch (burst_len) {
    case 1:    return DWAXIDMAC_BURST_TRANS_LEN_1;
    case 4:    return DWAXIDMAC_BURST_TRANS_LEN_4;
    case 8:    return DWAXIDMAC_BURST_TRANS_LEN_8;
    case 16:   return DWAXIDMAC_BURST_TRANS_LEN_16;
    case 32:   return DWAXIDMAC_BURST_TRANS_LEN_32;
    case 64:   return DWAXIDMAC_BURST_TRANS_LEN_64;
    case 128:  return DWAXIDMAC_BURST_TRANS_LEN_128;
    case 256:  return DWAXIDMAC_BURST_TRANS_LEN_256;
    case 512:  return DWAXIDMAC_BURST_TRANS_LEN_512;
    case 1024: return DWAXIDMAC_BURST_TRANS_LEN_1024;
    default:
				  ...
				  dma_addr_t mem_addr, size_t len)
{
	...
	u32 ctllo, ctlhi;
	u32 burst_len;
	bool src_incr = chan->pcfg.src_addr_incr;
	bool dst_incr = chan->pcfg.dst_addr_incr;
	...
	case DMA_MEM_TO_DEV:
		...
		ctllo = reg_width << CH_CTL_L_DST_WIDTH_POS |
			...
			(dst_incr ? DWAXIDMAC_CH_CTL_L_INC : DWAXIDMAC_CH_CTL_L_NOINC) << CH_CTL_L_DST_INC_POS |
	...
	case DMA_DEV_TO_MEM:
		...
		ctllo = reg_width << CH_CTL_L_SRC_WIDTH_POS |
			...
			(src_incr ? DWAXIDMAC_CH_CTL_L_INC : DWAXIDMAC_CH_CTL_L_NOINC) << CH_CTL_L_SRC_INC_POS;
	...
	ctllo |= dma_burst_len_to_enum(chan->pcfg.dst_fixed_burst) << CH_CTL_L_DST_MSIZE_POS |
		  dma_burst_len_to_enum(chan->pcfg.src_fixed_burst) << CH_CTL_L_SRC_MSIZE_POS;
					...
					struct dma_slave_config *config)
{
	...
	if (config->peripheral_config) {
		if (config->peripheral_size != sizeof(struct axi_dma_peripheral_config)) {
			dev_err(chan2dev(chan), "Invalid peripheral size %zu, expected %zu\n",
				config->peripheral_size, sizeof(struct axi_dma_peripheral_config));
			return -EINVAL;
		}
		memcpy(&chan->pcfg, config->peripheral_config, config->peripheral_size);
	}
