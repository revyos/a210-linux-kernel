...
#define VIDEO_MAPPING_IPI_EN BIT(15)
...
//#define AVERAGE_BYTES_PER_TU_FRAC		GENMASK(19, 14)
#define AVERAGE_BYTES_PER_TU_FRAC		GENMASK(19, 16) //sst mode only used 16~19
...
#define LANE0_SW_LEVEL				GENMASK(3, 2)
#define LANE1_SW_LEVEL				GENMASK(9, 8)
#define LANE2_SW_LEVEL				GENMASK(15, 14)
#define LANE3_SW_LEVEL				GENMASK(21, 20)

#define LANE0_PRE_LEVEL				GENMASK(1, 0)
#define LANE1_PRE_LEVEL				GENMASK(7, 6)
#define LANE2_PRE_LEVEL				GENMASK(13, 12)
#define LANE3_PRE_LEVEL				GENMASK(19, 18)
...
#define VIDEO_FIFO_UNDERFLOW_STREAM0	BIT(8)
...
static int __maybe_unused dw_dp_hdcp_init_keys(struct dw_dp *dp)
{
#if 0
	u32 val;
	int size;
...
static int __maybe_unused dw_dp_hdcp_rng_init(struct dw_dp *dp)
{
	u32 random_val;
...
static int __maybe_unused dw_dp_hw_hdcp_init(struct dw_dp *dp)
{
...
static bool __maybe_unused dw_dp_hdcp2_capable(struct dw_dp *dp)
{
	u8 rx_caps[3];
	int ret;
...
static bool __maybe_unused dw_dp_hdcp_capable(struct dw_dp *dp)
{
	int ret;
	u8 bcaps;
...
static int __maybe_unused dw_dp_hdcp_auth_check(struct dw_dp *dp)
{
	u32 val;
	int ret;
...
static int dw_dp_hdcp_check_link(struct dw_dp *dp)
{
	...
	dev_err(dp->dev, "HDCP link failed, retrying authentication\n");
...
static void __maybe_unused dw_dp_hdcp_init(struct dw_dp *dp)
{
...
static void dw_dp_handle_hdcp_event(struct dw_dp *dp)
{
	...
	if (value & HDCP22_GPIOINT) {
		dev_err(dp->dev, "A change in HDCP22 GPIO Output status\n");
...
static int __maybe_unused dw_dp_hdcp_atomic_check(struct drm_connector *conn,
...
static int dw_dphy_set_rate(struct dw_dp *dp, unsigned int rate)
{
	int val = 0;
	switch (rate) {
	case 162000:
	...
	regmap_update_bits(dp->regmap, DPTX_PHYIF_CTRL, PHY_RATE, FIELD_PREP(PHY_RATE, val));
	return 0;
}
...
static int dw_dphy_set_voltage_sw_level(struct dw_dp *dp, int lane_id, int level)
{
	...
	if (lane_id == 0) {
		regmap_update_bits(dp->regmap, DPTX_PHY_TX_EQ, LANE0_SW_LEVEL, FIELD_PREP(LANE0_SW_LEVEL, level));
	} else if (lane_id == 1) {
		regmap_update_bits(dp->regmap, DPTX_PHY_TX_EQ, LANE1_SW_LEVEL, FIELD_PREP(LANE1_SW_LEVEL, level));
	} else if (lane_id == 2) {
		regmap_update_bits(dp->regmap, DPTX_PHY_TX_EQ, LANE2_SW_LEVEL, FIELD_PREP(LANE2_SW_LEVEL, level));
	} else if (lane_id == 3) {
		regmap_update_bits(dp->regmap, DPTX_PHY_TX_EQ, LANE3_SW_LEVEL, FIELD_PREP(LANE3_SW_LEVEL, level));
	}
...
static int dw_dphy_set_voltage_pre_level(struct dw_dp *dp, int lane_id, int level)
{
	...
	if (lane_id == 0) {
		regmap_update_bits(dp->regmap, DPTX_PHY_TX_EQ, LANE0_PRE_LEVEL, FIELD_PREP(LANE0_PRE_LEVEL, level));
	} else if (lane_id == 1) {
		regmap_update_bits(dp->regmap, DPTX_PHY_TX_EQ, LANE1_PRE_LEVEL, FIELD_PREP(LANE1_PRE_LEVEL, level));
	} else if (lane_id == 2) {
		regmap_update_bits(dp->regmap, DPTX_PHY_TX_EQ, LANE2_PRE_LEVEL, FIELD_PREP(LANE2_PRE_LEVEL, level));
	} else if (lane_id == 3) {
		regmap_update_bits(dp->regmap, DPTX_PHY_TX_EQ, LANE3_PRE_LEVEL, FIELD_PREP(LANE3_PRE_LEVEL, level));
	}
...
static int __maybe_unused dw_dp_get_eotf(struct drm_connector_state *conn_state)
{
...
static void dw_dp_check_service_irq(struct dw_dp *dp)
{
	...
	if (val & DP_SINK_SPECIFIC_IRQ)
		dev_err(dp->dev, "Sink specific irq unhandled\n");
}
				 ...
				 struct hdmi_codec_params *params)
{
	...
	if (audio->format == AFMT_I2S)
		clk_disable_unprepare(dp->gtc_clk);
...
static int __maybe_unused dw_dp_register_audio_driver(struct dw_dp *dp)
{
	struct dw_dp_audio *audio = &dp->audio;
...
static void __maybe_unused dw_dp_unregister_audio_driver(void *data)
{
	struct dw_dp *dp = data;
	struct dw_dp_audio *audio = &dp->audio;
	return;
...
static int dw_dp_probe(struct platform_device *pdev)
{
	...
	dp->id = id;
	dp->dev = dev;
	//SST, Single Pixel Modes
	dp->video.pixel_mode = DPTX_MP_SINGLE_PIXEL;
	...
	dp->ipi_clk = devm_clk_get(dev, "ipi");
	if (IS_ERR(dp->ipi_clk))
		return dev_err_probe(dev, PTR_ERR(dp->ipi_clk),
	...
	dp->pclk = devm_clk_get_optional(dev, "pclk");
	if (IS_ERR(dp->pclk))
	...
	ret = devm_request_irq(dev, dp->irq, dw_dp_irq_handler, 0, dev_name(dev), dp);
	if (ret) {
...
static int __maybe_unused dw_dp_runtime_suspend(struct device *dev)
{
	...
	clk_disable_unprepare(dp->ipi_clk);
