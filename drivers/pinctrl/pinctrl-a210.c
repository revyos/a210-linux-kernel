...
static int a210_pinctrl_probe(struct platform_device *pdev)
{
	...
	const struct a210_pad_group *group = device_get_match_data(dev);
...
static int a210_pinctrl_suspend(struct device *dev)
{
	...
	switch(pp->type) {
		case A210_PADCTRL_AON:
			ret = a210_pinctrl_backup_regs(pp, A210_AON_CFG_REG_NUMS, A210_AON_MUX_REG_NUMS);
			break;
		case A210_PADCTRL_0:
			...
			break;
		case A210_PADCTRL_1:
			ret = a210_pinctrl_backup_regs(pp, A210_PADCTRL1_CFG_REG_NUMS, A210_PADCTRL1_MUX_REG_NUMS);
			clk_disable_unprepare(pp->clk);
			break;
		case A210_PADCTRL_2:
...
static int a210_pinctrl_resume(struct device *dev)
{
	...
	switch(pp->type) {
		case A210_PADCTRL_AON:
			ret = a210_pinctrl_restore_regs(pp, A210_AON_CFG_REG_NUMS, A210_AON_MUX_REG_NUMS);
			break;
		case A210_PADCTRL_0:
			...
			ret = a210_pinctrl_restore_regs(pp, A210_PADCTRL0_CFG_REG_NUMS, A210_PADCTRL0_MUX_REG_NUMS);
			break;
		case A210_PADCTRL_1:
			ret = clk_prepare_enable(pp->clk);
			if (ret) {
			...
			ret = a210_pinctrl_restore_regs(pp, A210_PADCTRL1_CFG_REG_NUMS, A210_PADCTRL1_MUX_REG_NUMS);
			break;
		case A210_PADCTRL_2:
			ret = clk_prepare_enable(pp->clk);
			if (ret) {
			...
			ret = a210_pinctrl_restore_regs(pp, A210_PADCTRL2_CFG_REG_NUMS, A210_PADCTRL2_MUX_REG_NUMS);
			break;
		default:
			break;
	}
...
static const struct of_device_id a210_pinctrl_of_match[] = {
	{ .compatible = "zhihe,a210-group0-pinctrl", .data = &a210_group0 },	/* aon_padctrl */
	{ .compatible = "zhihe,a210-group1-pinctrl", .data = &a210_group1 },	/* peri1_padctrl */
	{ .compatible = "zhihe,a210-group2-pinctrl", .data = &a210_group2 },	/* peri2_padctrl */
	{ .compatible = "zhihe,a210-group3-pinctrl", .data = &a210_group3 },	/* peri3_padctrl */
	{ /* sentinel */ }
};
