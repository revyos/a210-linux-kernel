...
static int i2c_dw_hwparams_to_dma_slave_config(struct dw_i2c_dev *dev)
{
    ...
    struct axi_dma_peripheral_config pconfig = {0};
    ...
    pconfig.dst_addr_incr = dma->txdst_incr;
    pconfig.src_addr_incr = dma->rxsrc_incr;
    pconfig.src_fixed_burst = dma->rxburst_len;
    pconfig.dst_fixed_burst = dma->txburst_len;
    slave_config.peripheral_config = &pconfig;
    slave_config.peripheral_size = sizeof(pconfig);
...
static void i2c_dw_dma_callback(void *data)
{
    ...
    dma->tx_status &= ~STATUS_WRITE_IN_PROGRESS;
