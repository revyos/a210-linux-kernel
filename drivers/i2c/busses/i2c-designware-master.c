...
i2c_dw_xfer(struct i2c_adapter *adap, struct i2c_msg msgs[], int num)
{
	...
	/* Wait for dma tx to complete if dma enabled*/
    if (dev->dma_enable) {
	...
	if (dev->dma_enable && dev->dma.tx_status) {
		dev_err(dev->dev, "transfer terminated early - interrupt latency too high? status 0x%x, tx_status 0x%x\n", dev->status, dev->dma.tx_status);
		dev_err(dev->dev, "laststa 0x%x, laststatus 0x%x,rx_outstanding %d\n", dev->dma.laststat, dev->dma.laststatus, dev->rx_outstanding);
		if(dev->dma.laststat & DW_IC_INTR_RX_OVER) {
			ret = -EAGAIN;
			goto done;
		}
	}
...
tx_aborted:
	...
	if ((stat & (DW_IC_INTR_TX_ABRT|DW_IC_INTR_RX_OVER))|| dev->msg_err ||
		...
		dev->dma.laststat = stat;
		dev->dma.laststatus = status;
...
int i2c_dw_probe_master(struct dw_i2c_dev *dev)
{
	...
	dev->dma_enable = of_property_read_bool(np, "dma-mode");;
	if (dev->dma_enable) {
		const __be32 *prop;

		memset(&dev->dma, 0, sizeof(dev->dma));
		/* Get dma tx/rx address Increment */
		dev->dma.txdst_incr = of_property_read_bool(np, "dma-tx-addr-incr");
		dev->dma.rxsrc_incr = of_property_read_bool(np, "dma-rx-addr-incr");

		prop = of_get_property(np, "dma-burst-len", NULL);
		if (!prop) {
			dev_info(dev->dev, "Failed to get 'dma-burst-len' property, use 0 default.\n");
		} else {
			ret = of_property_read_u32_index(np, "dma-burst-len", 0, &dev->dma.txburst_len);
			if (ret) {
				dev_err(dev->dev, "Failed to read tx burst len of 'dma-burst-len'\n");
				return ret;
			}
			ret = of_property_read_u32_index(np, "dma-burst-len", 1, &dev->dma.rxburst_len);
			if (ret) {
				dev_err(dev->dev, "Failed to read rx burst len of 'dma-burst-len'\n");
				return ret;
			}
		}
	}
